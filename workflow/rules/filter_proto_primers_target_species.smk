...
rule format_primers_into_fasta:
    ...
    log: "logs/{species}.{pool}.filter_primers.log"
    conda:
        "../envs/primers.yaml"
    shell:
...
rule create_index_for_target_species:
    ...
    params:
        outdir = "tmp/index/"
    log: "logs/{species}.filter_primers.log"
...
rule align_primers_to_species:
    input: 
        primers = "results/{species}.{pool}.proto_primers.fasta",
        temp(expand("tmp/index/{{species}}.{version}.ebwt", version=range(1, 4))),
        temp(expand("tmp/index/{{species}}.rev.{version}.ebwt", version=range(1, 2)))
    ...
    conda: "../envs/bowtie.yaml"
    shell:
        "python scripts/align_primers.py --primers_input {input} --index_input {params.index} --output_file {output} >> {log} 2>&1"

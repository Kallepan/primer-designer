...
class PrimerGenerator():
    ...
    def __init__(self, region_name: str, amplicon_index: int, amplicon_sequence: Seq, amplicon_overlap: int, primer3_settings: str, temp_dir: str, iteration_index: int):
        self._primer3_settings = primer3_settings
        self.amplicon_sequence = amplicon_sequence
        self.amplicon_overlap = amplicon_overlap
    ...
    def __write_temp_primer_gen_file(self) -> str:
        PRIMER3_OK_REGION_LIST=f"0,{self.amplicon_overlap},{len(self.amplicon_sequence) - self.amplicon_overlap},{self.amplicon_overlap}"

...
def __get_region_df(db: DBHandler) -> pd.DataFrame:
    regions, column_names = db.select(""" SELECT name, start, end FROM regions ORDER BY name ASC""")
    region_df = pd.DataFrame(regions, columns=column_names, dtype="string")
    
    return region_df

def __to_json(region_df: pd.DataFrame, args: argparse.Namespace) -> None:
    region_df.to_json(args.output, orient="records")

def main():
    logging.info("Formatting regions into a json file and writing to database")
    args = __get_args()
    db = DBHandler(args.db)
    regions = __get_regions(args.regions)
    __to_db(regions, db)
    region_df = __get_region_df(db)
